...
def load_and_preprocess_data(directory):
    ...
    for class_name, label in zip(['basil', 'ginseng'], [0, 1]):
        ...
        for image_name in os.listdir(class_dir):
            ...
            img = img.flatten() / 255.0  # 이미지 스케일링 (0에서 1 사이 값으로) 및 1차원 벡터로 변환
            X.append(img)
            y.append(label)
...
result_label = tk.Label(frame, text=f'훈련 데이터셋 이미지 수: {num_train_images}, 검증 데이터셋 이미지 수: {num_val_images}')  # 결과 텍스트 레이블
